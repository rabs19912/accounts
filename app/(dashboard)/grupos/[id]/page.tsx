...
export default async function GroupPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const myBalances = computeMyBalances(userId, memberIds, expenseInputs, loanInputs);
  ...
  // Miembros pendientes (invitados que no aceptaron)
  ...
  // Usuarios disponibles para invitar (no miembros ni pendientes)
  const availableUsers = allUsers.filter(
    (u) => !memberIds.includes(u.id) && !pendingUserIds.includes(u.id)
  );

  // Otros miembros (para el selector de préstamo)
